...
public:
	...
	void SetProtonDecayData2(G4String theFileName);
        G4double GetProtonEng(); // Randomly chooses Proton ExEng
        G4double GetProtonEng2(G4double Energy);
        void SetExcitationEnergy(G4double energy){Excitation_Energy = energy;}
        G4double GetExcitationEnergy(){return Excitation_Energy;} // Randomly chooses Proton ExEng
...
private:
  ...
  std::vector <std::vector<G4double> > Energy_table;
  std::vector <std::vector<G4double> > Probability_table;
  std::vector <std::vector<G4double> > Limits_table;
