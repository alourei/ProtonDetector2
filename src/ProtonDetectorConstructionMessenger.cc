...
ProtonDetectorConstructionMessenger::ProtonDetectorConstructionMessenger(ProtonDetectorConstruction *det):ProtonDetector(det) {
	...
	radiusInnerPadCmd = new G4UIcmdWithADoubleAndUnit("/ProtonDetector/det/setInnerPadRadius",this);
	radiusInnerPadCmd->SetGuidance("Selects the radius of the Inner Pad.");
	radiusInnerPadCmd->SetParameterName("radiusInnerPad",false);
	radiusInnerPadCmd->SetRange("radiusInnerPad>=0.");
	radiusInnerPadCmd->SetUnitCategory("Length");
	radiusInnerPadCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

	radiusOuterPadCmd = new G4UIcmdWithADoubleAndUnit("/ProtonDetector/det/setOuterPadRadius",this);
	radiusOuterPadCmd->SetGuidance("Selects the radius of the Outer Pad.");
	radiusOuterPadCmd->SetParameterName("radiusOuterPad",false);
	radiusOuterPadCmd->SetRange("radiusOuterPad>=0.");
	radiusOuterPadCmd->SetUnitCategory("Length");
	radiusOuterPadCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
ProtonDetectorConstructionMessenger::~ProtonDetectorConstructionMessenger() {
	...
	delete radiusInnerPadCmd;
	delete radiusOuterPadCmd;
	//Degrader settings
...
void ProtonDetectorConstructionMessenger::SetNewValue(G4UIcommand *command, G4String newValue){
	...
	if(command == radiusInnerPadCmd){
		ProtonDetector->SetRadiusInnerPad(radiusInnerPadCmd->GetNewDoubleValue(newValue));
	}

	if(command == radiusOuterPadCmd){
		ProtonDetector->SetRadiusOuterPad(radiusOuterPadCmd->GetNewDoubleValue(newValue));
	}
