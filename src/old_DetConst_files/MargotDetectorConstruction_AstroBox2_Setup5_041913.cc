...
//
//
// $Id: MargotDetectorConstruction.cc,v 1.9 2013/04/19 17:47:19 roeder Exp $
// GEANT4 tag $Name: geant4-09-03 $
//
// Modified by Brian Roeder, TAMU on 04/19/2013
...
MargotDetectorConstruction::MargotDetectorConstruction(G4String DetType, G4double Thick, G4double DegRot)
 :solidWorld(0),  logicWorld(0),  physiWorld(0),
  ...
  solidMicroBulk(0), logicMicroBulk(0), 
  physiMicroBulk1(0), physiMicroBulk2(0), physiMicroBulk3(0), physiMicroBulk4(0), physiMicroBulk5(0),
...
G4VPhysicalVolume* MargotDetectorConstruction::Construct()
{
  ...
  //G4double PressureFactor = 0.5000;  // 1/2 atm
  //G4double PressureFactor = 0.789474;  // 600 torr
 ...
 // Updated 2/13/2012 to be a "Union solid", changed to be
 // retangular for AstroBox2!
  // ====> Setup #5 dimensions =================  

  // ----- New Dimensions for AstroBox2 as calculated 4/19/2013

  // Solid for the length of the chamber - a box for AstroBox2
  // Right now only considers "field cage"
  // box is 185.5 long (along z - beam axis)
  //        40mm  wide (along x - detection, horizontal axis)
  //        40mm  high (along y-vertical axis)

  // Assume same circular inlets as from AstroBox1.
  // Pollacco requests det. center at 120mm from window.
  // This adds 17.25 mm to previous setup #3 dimension estimates.

  G4double IonChamberP1x = 45.0*mm;
  G4double IonChamberP1y = 110.0*mm;
  G4double IonChamberP1z = 220.0*mm;   
  ...
  solidIonChamberP1 = new G4Box("IonChamberP1",IonChamberP1x/2.,IonChamberP1y/2.,IonChamberP1z/2.);
    ...
    G4double halfHeightICP2 = (240.0/2.)*mm;
 ...
 // Visualization (is it there?)
  ...
  // The chamber interior with gas
    ...
    G4double ICGasP1x = 40.0*mm;
    G4double ICGasP1y = 105.0*mm;
    G4double ICGasP1z = 215.00*mm;
    ...
    G4double halfHeightICGasP2 = ((240.00/2.))*mm;
...
// Ion Chamber Window
 ...
 // Now simulated as "Box" Volumes
  ...
  // -----------------------------------------
  // New Pad Dimensions, Calculated 4/19/2013 (as per email 4/17/2013)
  // Now there are 5 MicroBulk pads, with Dims. of Setup 3
  // -----------------------------------------
    G4double MicroBulkX = 40.0*mm;
    G4double MicroBulkY = 40.0*mm;
    G4double MicroBulkZ = 20.0*mm;

    G4double DeadZoneGapSize = 1.0*mm;
    if(DetectorType == "Al23_GasIC_NoGap")
      {DeadZoneGapSize = 0.*mm;}
    else if(DetectorType == "Al23_GasIC_wGap")
      {DeadZoneGapSize = 0.1*mm;}
    ...
    G4ThreeVector positionMicroBulk1 = G4ThreeVector(0.0,0.0,-2.0*(MicroBulkZ+DeadZoneGapSize)); // w.r.t. Ion Chamber Gas
    ...
    G4ThreeVector positionMicroBulk2 = G4ThreeVector(0.0,0.0,-1.0*(MicroBulkZ+DeadZoneGapSize)); // w.r.t. Ion Chamber Gas
    ...
    G4ThreeVector positionMicroBulk3 = G4ThreeVector(0.0,0.0,0.0*(MicroBulkZ+DeadZoneGapSize)); // w.r.t. Ion Chamber Gas
    ...
    G4ThreeVector positionMicroBulk4 = G4ThreeVector(0.0,0.0,1.0*(MicroBulkZ+DeadZoneGapSize)); // w.r.t. Ion Chamber Gas

    physiMicroBulk4 = new G4PVPlacement(0,                // no rotation
				      positionMicroBulk4, // position defined above
				      logicMicroBulk,    // its logical volume
                                      "MicroBulk4",       // its name
				      logicIonChamberGas,  // its mother  volume
				      false,              // no boolean operations
 ...
 G4ThreeVector positionMicroBulk5 = G4ThreeVector(0.0,0.0,2.0*(MicroBulkZ+DeadZoneGapSize)); // w.r.t. Ion Chamber Gas

    physiMicroBulk5 = new G4PVPlacement(0,                // no rotation
				      positionMicroBulk5, // position defined above
				      logicMicroBulk,    // its logical volume
                                      "MicroBulk5",       // its name
				      logicIonChamberGas,  // its mother  volume
				      false,              // no boolean operations
 ...
 // Visualization (is it there?)
  ...
  G4double BulkTBx = MicroBulkX;
  G4double BulkTBy = 20.0*mm;  // Total active Y length = 70mm, so 70mm-30mm(MicroBulk) = 40.mm/2 (top/bottom
  G4double BulkTBz = (MicroBulkZ*2.5+20.)*mm;  // 20.0*2.5+20, see MicroBulkz above
    ...
    G4ThreeVector positionBulkTop1 = G4ThreeVector(0.0,(BulkTBy/2.+MicroBulkY/2.+1.0*mm),-(15.0*mm+0.5*mm+MicroBulkZ)); // w.r.t. Ion Chamber
    ...
    G4ThreeVector positionBulkTop2 = G4ThreeVector(0.0,(BulkTBy/2.+MicroBulkY/2.+1.0*mm),(15.0*mm+0.5*mm+MicroBulkZ)); // w.r.t. Ion Chamber
    ...
    G4ThreeVector positionBulkBottom1 = G4ThreeVector(0.0,-(BulkTBy/2.+MicroBulkY/2.+1.0*mm),-(15.0*mm+0.5*mm+MicroBulkZ)); // w.r.t. Ion Chamber
    ...
    G4ThreeVector positionBulkBottom2 =  G4ThreeVector(0.0,-(BulkTBy/2.+MicroBulkY/2.+1.0*mm),(15.0*mm+0.5*mm+MicroBulkZ)); // w.r.t. Ion Chamber
  ...
  G4double BulkFBx = MicroBulkX;
  G4double BulkFBy = MicroBulkY;  // Total active Y length = 70mm, so 70mm-30mm(MicroBulk) = 40.mm/2 (top/bottom)
  G4double BulkFBz = 20.0*mm;
    ...
    G4ThreeVector positionBulkFront = G4ThreeVector(0.0,0.0,-(1.0*mm+2.5*MicroBulkZ+BulkFBz/2.)); // w.r.t. Ion Chamber
    ...
    G4ThreeVector positionBulkBack = G4ThreeVector(0.0,0.0,(1.0*mm+2.5*MicroBulkZ+BulkFBz/2.)); // w.r.t. Ion Chamber
 ...
 // Visualization (is it there?)
  ...
  G4double BeamCounterX = MicroBulkX;
  ...
  G4double BeamCounterZ = 10.0*mm;
    ...
    G4ThreeVector positionBeamCounterFront = G4ThreeVector(0.0,0.0,-(1.0*mm+2.5*MicroBulkZ+BulkFBz+BeamCounterZ/2.)); // w.r.t. Ion Chamber
    ...
    G4ThreeVector positionBeamCounterBack = G4ThreeVector(0.0,0.0,(1.0*mm+2.5*MicroBulkZ+BulkFBz+BeamCounterZ/2.)); // w.r.t. Ion Chamber
